...
### RULE-12 | 외부 제휴 API 클라이언트 직접 호출 절대 금지 (Server-Side Proxy 강제)
- **위험성:** 앱(Client)에서 쿠팡 API 엔드포인트(`api-gateway.coupang.com`)를 직접 호출할 경우, HMAC 서명 키가 탈취될 위험이 있으며 잦은 호출로 인해 파트너스 계정이 영구 정지될 수 있다.
- **로직:** 모든 외부 API 요청은 반드시 Firebase Cloud Functions(서버)를 경유해야 한다. 클라이언트 단의 `fetch`나 `axios`에 외부 도메인이 직접 하드코딩되는 것을 원천 차단한다.
- **캐싱(Caching) 강제 규칙:** 파트너스 API 가이드라인에 의거, **'모바일 앱 캐시 만료 시간 60분'**을 엄수한다. 랭킹 데이터(`bestcategories`)는 1시간에 한 번만 쿠팡 서버에서 긁어와 Firestore에 캐싱(`products_cache` 컬렉션)해 두고, 유저들에게는 캐싱된 데이터를 서빙한다.
...
🏠 [ZONE C] 메인 탭 구역 (GNB 5-Tabs)
  ...
  - [C-2] 랭킹 탭 (RankingScreen): 완료 ✅ (초개인화 핀테크 대시보드 개편 완)
      ├─ [C-2-1] 글로벌 헤더 및 검색 (Header Sync):
      │    ├─ [UI]: 홈 탭과 완벽히 동일한 헤더 레이아웃 유지 (가짜 검색바 + 우측 🔔 알림 아이콘).
      │    └─ [라우팅]: 검색바 터치 시 인플레이스 키보드 노출 방지, 전역 `SearchScreen`으로 라우팅.
      ├─ [C-2-2] 생애주기 맞춤 동적 카테고리 탭 (Dynamic Category):
      │    ├─ [기본 정렬]: 범용성 극대화를 위해 `[생활용품(1014)] ➔ [식품(1012)] ➔ [출산/유아동(1011)]` 순서 기본 적용.
      │    ├─ [자동 스위칭]: 자녀 월령 36개월 이상 유저의 경우 `출산/유아동` 탭을 `[유아동패션(1030)]` 또는 `[완구(1020)]`로 완전 교체.
      │    └─ [동적 라벨링]: 쿠팡 `1012(식품)` 카테고리 노출 시, 월령 < 12개월은 "식품/분유", 월령 >= 12개월은 "식품/간식"으로 텍스트 치환.
      ├─ [C-2-3] 가격 하락 티커 배너 (Ticker Banner):
      │    ├─ [UI]: 카테고리 바로 아래 연초록 배경(`backgroundColor: '#F0FDF4'`)의 주식 앱 스타일 배너 배치. 잉여 여백(Margin) 강제 압착.
      │    ├─ [동적 텍스트]: `현재 [${선택된_동적_카테고리명}] 제품 중 N개 상품의 가격이 어제보다 하락했어요.`
      │    └─ [라우팅]: 터치 시 `CurationDetailScreen` 이동 (파라미터: `{ type: 'price_drop', title: '오늘의 가격 하락템' }`).
      ├─ [C-2-4] 또래 맞춤 토글 & 데이터 방어 로직 (Peer-Match & Fallback):
      │    ├─ [코치마크]: `position: 'absolute'`로 레이아웃 밀림 방지. "내 아이와 비슷한 또래 및 육아 환경에 맞춰 랭킹을 추천해 드려요." (AsyncStorage 활용 1회성 노출, 터치 시 영구 해제, 이모지 금지).
      │    ├─ [랭킹 기준 모달]: "랭킹 기준 ℹ️" 알약 뱃지 클릭 시 커스텀 바텀 시트 노출 (RULE-9.4 준수, Native Alert 절대 금지).
      │    │    └─ 워딩: [전체 랭킹] "쿠팡 판매 데이터를 바탕으로 선정한 베스트 상품" / [또래 맞춤 랭킹] "내 아이와 비슷한 또래 부모님들이 실제 가장 많이 선택하고 인정한 상품 (육아 환경 + 관심사 반영)"
      │    └─ [데이터 방어]: `[또래 맞춤]` 활성화 시 데이터 부족 시, 빈 화면 대신 노란색 핀테크 카드 배너(ℹ️ 아이콘 포함) 노출. ("⚠️ 아직 또래 맘들의 데이터가 모이고 있어요! 우선 쿠팡 전체 랭킹을 보여드릴게요.")
      └─ [C-2-5] 상품 리스트 UI 밀도 압착 및 렌더링 (Density & Pagination):
           ├─ [UI 구분선]: 카드 간 1px 연회색 실선(Hairline Divider) 적용 및 세로 여백 최소화로 데이터 밀도 극대화.
           ├─ [로켓배송 태그]: 가격 영역 우측에 `[할인율] [현재가] [🚀 로켓배송]` 순으로 인라인(Inline) 수평 배치 (하단 고립 금지).
           ├─ [무한 스크롤]: 10개씩 페이징 로드(Pagination). 하단 도달 시 ActivityIndicator(스마트 블루) 노출.
           └─ [액션 라우팅]: 개별 상품 터치 시 `product_click` 로그 기록 후 `ProductDetailScreen` [D-3]으로 즉시 이동.

  - [C-3] 커뮤니티 탭 (CommunityListScreen): 대기 중 ⏳ (이모지 철거 및 핀테크형 리스트 개편 필요)- [C-3] 커뮤니티 탭 (Community 2.0 & C2C Engine): 완료 ✅
      ├─ [C-3-1] 글로벌 헤더 및 커뮤니티 전용 검색 라우팅:
      │    └─ 상단 🔍 검색바 터치 시 홈 탭의 글로벌 검색이 아닌 커뮤니티 전용 `CommunitySearchScreen` [C-S]로 라우팅 (컨텍스트 분리).
      ├─ [C-3-2] 네이버 카페형 필터 및 탭 바:
      │    ├─ 카테고리: `[전체]`, `[육아수다]`, `[질문/고민]`, `[육아꿀템]`, `[특가제보]` (가독성 위해 폰트 굵기 400/500, 컬러 #334155 적용).
      │    └─ [내 아이 맞춤 정보] 토글 (우측 압착 레이아웃): 활성화 시 `authorBabyMonthAtCreation` 기준 동적 구간화 로직 가동.
      ├─ [C-3-3] 게시글 리스트 렌더링 (C2C 직관화):
      │    └─ 게시글 내 `taggedProduct` 존재 시, 제목 바로 아래에 `[ 🔗 브랜드명 - 상품명 ]` 형태의 상품 태그 칩(Pill)을 즉각 노출하여 커머스 연결성 강화.
      └─ [C-3-4] 글쓰기(FAB) 라우팅: 터치 시 `WritePostScreen` 진입.

  - [C-S] 커뮤니티 전용 로컬 검색 (CommunitySearchScreen): 완료 ✅ (신설)
      ├─ [UI/상태]: 진입 시 빈 화면(Empty State)에 "검색어를 입력해보세요" 렌더링. 1글자 이상 입력 시 실시간 리스트 필터링.
      ├─ [3-Way 탭 검색]: `[제목+내용]` / `[작성자]` / `[상품태그]` 탭 분리 (상품태그 탭은 연결된 상품명으로 후기 역추적).
      ├─ [데이터 제어]: 검색 볼륨 확보를 위해 진입 시 '내 아이 맞춤 정보(Peer Match)' 필터는 무조건 강제 해제(OFF) 상태로 쿼리.
      └─ [정렬 로직]: `최신순`, `댓글순`, `조회순`, `좋아요순` 실시간 동적 정렬.

  - [C-3-W] 글쓰기 화면 (WritePostScreen): 완료 ✅
      ├─ [상단/제목]: 대형 여백 제거. 카테고리 이모지 철거(알약 형태). 제목은 40자 하드 캡(`maxLength={40}`) 및 카운터 노출.
      ├─ [이미지 영역]: 리스트 최좌측에 당근마켓 스타일의 `[ 📷 0/5 ]` 스켈레톤 박스 상시 노출하여 어포던스 확보.
      └─ [C2C 상품 태그 툴바]: 하단에 파란색 `[🔗 상품 검색/태그]` 버튼 배치.
           └─ 클릭 시 [쿠팡 상품 검색 모달] 스르륵 노출 ➔ 검색 및 선택 시 에디터 하단에 '선택된 상품 미니 카드' 첨부.
...
🔍 [ZONE D] 공통 상세 구역 (Detail Views)
  - [D-1] CurationDetailScreen (만능 도화지 A - 테마별 큐레이션): 완료 ✅ (오늘의 가격 하락템 기준)
      ├─ [D-1-1] 명칭 및 상단 헤더:
      │    ├─ [브랜딩]: "폭락템" 등 유치한 단어 원천 금지 ➔ 공식 명칭 **"오늘의 가격 하락템"** 강제.
      │    ├─ [동적 타이틀]: 이전 화면(홈/랭킹)에서 전달받은 `route.params.title`을 헤더 타이틀로 동적 렌더링.
      │    └─ [UI 철거]: 우측 상단 리스트/그리드 뷰 전환 아이콘 삭제 (단일 리스트 뷰 강제하여 인지 부하 감소).
      ├─ [D-1-2] 핀테크 맞춤형 정렬 필터 (Sorting Logic):
      │    ├─ [쓰레기 옵션 철거]: 목적과 무관한 '최신순', '오래된순', '즐겨찾기순' 전면 삭제.
      │    └─ [허용 옵션 3종]: 
      │         1. `할인율순`: 어제 대비 가격 낙폭(%) 최대치 (진입 시 Default 강제).
      │         2. `낮은가격순`: 절대적 가격 수치 최저가.
      │         3. `판매량순`: 하락폭 무관 쿠팡 API 기준 인기/구매 높은 순서.
      └─ [D-1-3] Empty State (빈 화면 방어):
           └─ [UI 개편]: RULE-9.2 준수하여 네이티브 이모지(📬) 철거. `lucide-react-native`의 `<TrendingDown />` 또는 `<PackageX />` 벡터 아이콘과 회색 안내 텍스트로 대체 렌더링.
...
- **데이터 방어 (Fallback):** [또래 맞춤] 토글 시 내부 행동 데이터가 부족할 경우, 빈 화면 대신 노란색 핀테크 카드 배너(ℹ️ 아이콘 포함)를 노출한다. (워딩: "아직 또래 맘들의 데이터가 모이고 있어요! 우선 쿠팡 전체 랭킹을 보여드릴게요.")
...
**CTA Button 및 API 맵핑 로직 (수익화 직결 구역):**
- 라벨: **"쿠팡에서 최저가 확인하기"**
- Sticky (Footer 고정, `useSafeAreaInsets` 적용하여 플로팅 현상 방지)
- **[핵심 API 연동] 딥링크 컨버전 (`POST /deeplink`):**
  1. 유저가 CTA 버튼 클릭 시 `user_product_actions` 에 `product_purchase_click` 로깅.
  2. 백그라운드에서 해당 상품 URL을 파트너스 `POST /deeplink` API에 던져 트래킹 URL로 변환.
  3. 변환된 URL을 `Linking.openURL(affiliateUrl)`로 실행 (RULE-02 준수).
- **[핵심 API 연동] 배송 메타데이터 (`isRocket`):** - 쿠팡 API에서 전달받은 `isRocket: true/false`, `deliveryType` 필드를 파싱하여 최상단 가격 정보 란에 이모지가 배제된 동적 텍스트 태그(예: 파란색 `로켓배송`, 초록색 `로켓프레시`)로 렌더링한다.
- **[핵심 API 연동] 대안 상품 추천 (`GET /products/search`):**
  - Section 4(다른 대안 상품) 영역 렌더링 시, 현재 상품의 핵심 키워드를 추출하여 쿠팡 검색 API를 호출, 동종 카테고리의 핫딜 상품을 캐러셀로 노출한다.
...
**목적:** 홈 탭의 골드박스/가성비 특가 전체보기 및 랭킹 탭의 '가격 하락 배너' 클릭 시 진입하는 테마별 상품 대시보드.

**[1] 오늘의 가격 하락템 명세 (브랜딩 준수)**
- **공식 명칭:** 브랜드 신뢰도를 깎아먹는 "폭락템" 등 유치한 단어를 원천 금지하고 네비게이션 헤더 및 내부 텍스트를 **"오늘의 가격 하락템"**으로 통일한다.
- **뷰 토글 철거:** 우측 상단의 리스트/그리드 전환 아이콘을 완전히 삭제하여 인지 부하를 줄이고 단일 리스트 뷰 렌더링을 강제한다.

**[2] 핀테크 맞춤형 정렬 필터 (Sorting Logic)**
- 목적과 무관한 쓰레기 정렬 옵션(최신순, 오래된순, 즐겨찾기순 등)을 팝업에서 전면 철거한다.
- **허용된 정렬 옵션 3가지:**
  1. **할인율순 (Default 강제):** 진입 시 최우선 적용. 어제 대비 가격 낙폭(%)이 가장 큰 순서.
  2. **낮은가격순:** 절대적 가격 수치가 저렴한 순서.
  3. **판매량순:** 하락폭과 무관하게 쿠팡 API 기준 실제 구매/인기가 높은 순서.

**[3] Empty State (빈 화면 UI 방어 및 이모지 철거)**
- 조건에 맞는 하락 상품이 없을 때 노출되는 빈 화면(Empty State)에서 네이티브 우체통 이모지(📬) 사용을 **[RULE-9.2]**에 의거 절대 금지한다.
- `lucide-react-native` 패키지의 `<TrendingDown />` 또는 `<PackageX />` 등 세련된 벡터 아이콘과 회색 안내 텍스트로 대체 렌더링한다.
