...
const ONBOARDING_KEY = 'HAS_FINISHED_ONBOARDING';
...
// ─── Screen ───────────────────────────────────────────────────────────────────
export default function InitialOnboardingScreen({ onComplete, onSkip }) {
  ...
  const startAnalysis = () => {
    ...
    setTimeout(async () => {
      const childData = {
        type: babyStatus === 'born' ? 'child' : babyStatus === 'pregnant' ? 'pregnancy' : 'planning',
        firstName: (babyStatus === 'born' ? firstName : taemyeong).trim(),
        lastName: lastName.trim(),
        gender: gender === '여아' ? 'female' : gender === '남아' ? 'male' : null,
        ageMonth: dob ? computeAgeMonth(dob) : null,
      ...
      await AsyncStorage.multiSet([
        [ONBOARDING_KEY, 'true'],
        ['ONBOARDING_CHILD_DATA', JSON.stringify(childData)],
      ]).catch(() => {});
      if (onComplete) { onComplete(); } else { navigation.navigate('홈'); }
    }, 2400);
  };
      ...
      >
        ...
        {step === 3 && !skipPhysical && (
          <View>
            ...
            <Text style={styles.physicalHint}>정확하지 않아도 괜찮아요! 대략적인 수치로 맞춤 상품을 찾아드릴게요.</Text>
            <View style={styles.fieldGroup}>
...
const styles = StyleSheet.create({
  ...
  // Consent (Step 1)
  consentWrap:          { flex: 1, paddingTop: 40 },
  consentTitle:         { fontSize: 26, fontWeight: '900', color: NAVY, lineHeight: 36, marginBottom: 12 },
  consentSubtitle:      { fontSize: 15, color: GRAY, lineHeight: 22, marginBottom: 36 },
  consentBlock:         { gap: 20 },
  consentItem:          { flexDirection: 'row', alignItems: 'center', gap: 14 },
  consentCheckbox:      { width: 24, height: 24, borderRadius: 12, borderWidth: 2, borderColor: '#cbd5e1', backgroundColor: 'transparent' },
  consentCheckboxActive:{ borderColor: ACCENT, backgroundColor: ACCENT },
  consentItemText:      { fontSize: 16, color: '#334155', flex: 1 },
  ...
  physicalHint: { fontSize: 13, color: '#64748b', lineHeight: 18, marginBottom: 20 },
